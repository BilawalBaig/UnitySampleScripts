...
public class RoomController : Photon.MonoBehaviour
{
    ...
    int waitBeforeRespawn = 5; //How much seconds player need to wait before respawn again
	...
	IEnumerator Start ()
    {
		...
		PostActivityRemote( "", PhotonNetwork.player.name + " joined room", "", 0, 0);
	...
	void FixedUpdate ()
    {
        ...
        else
        {
			...
			currentRoundTime = 15 - Mathf.Round(((float)PhotonNetwork.time - referenceTime) * 10) / 10;
    ...
    { 
		...
		PostActivityRemote( "", newPlayer.name + " joined room", "", 0, 0);
	}
    ...
    { 
		...
		PostActivityRemote( "", otherPlayer.name + " left room", "", 0, 0);
	}
